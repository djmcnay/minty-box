...
class TestCompileFillerPattern:
    ...
    def test_pattern_matches_filler_words(self, mock_whisper_model: MagicMock) -> None:
        with (
            ...
            # Default list is non-lexical hesitations only — content
            # words like "I mean" / "like" / "actually" are not stripped.
            assert not p.search("I mean, that's fine")
            assert not p.search("I like coffee")
